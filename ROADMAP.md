...
- grid exists
- grid uses compact cell sizing and a larger default visible sheet footprint
- formula bar exists
- side inspector exists
- Sheet tabs sit below the formula bar and above the grid
...
- mouse click selection and double-click cell editing are usable without clearing cell contents
- Excel-style range formulas and `IF` support exist
- `ROUNDUP`, exact-match `VLOOKUP`, exact/default `XLOOKUP`, cross-Sheet lookup ranges, and helper-key lookup formulas exist as normal calculation primitives
...
- sticky row and column headers exist for grid scrolling
...
- dropdown inputs render in both the grid and Runner Preview
- short embedded dropdown options are configured on input Smart Cells
- imported supported Excel dropdowns are promoted to surfaced input Smart Cells
- action Smart Cell role exists
- multi-criteria lookup editor exists
- Help tab exists and now includes deeper workflow/formula/import/dropdown guidance
...
- add a small inspector color palette for normal and Smart Cells as non-operational cell formatting; start with single-cell coloring only, not range selection or formula behavior
...
- `Import Workbook` button exists in the toolbar
...
- simple imported Excel data-validation lists and supported same-workbook range/named-range sources can become embedded dropdown options
- supported imported Excel dropdown cells can be promoted to surfaced input Smart Cells with generated safe names
- unique Smart Cell names are workbook-scoped and can be referenced directly across Sheets
- exact lookup formulas are preserved as formula cells and can calculate without lookup Smart Cell promotion
- approximate lookup behavior is preserved for review
- named ranges and unsupported/risky formula patterns produce review items
- merged Excel ranges are detected and reported; covered cells are not silently duplicated
- basic cross-Sheet formulas calculate across preserved Sheets
- Runner Preview gathers surfaced Smart Cells from the full workbook and groups them by Sheet
- import review is shown before confirmation; completed imports return focus to the Sheet
- historical generated fixture workbooks, manual browser fixture results, and first-pass import planning docs are archived under `docs/archive/import-testing/`
- private GitHub baseline is established before the next import model changes

Next work:

- refine Sheet controls and naming after browser testing
...
- improve unsupported Excel dropdown source review and repair affordances
- expand formula compatibility based on real imported calculators
- define the next manual testing approach before regenerating or adding workbook fixtures
...
- empty runner sections are hidden
- Help tab explains the core concepts
- Help tab includes supported formula syntax, definitions, examples, dropdown behavior, import behavior, and keyboard shortcuts
...
- Dropdown input Smart Cells should be able to use visible reference data as option sources for longer lists.
- Normal exact lookup formulas should keep working as formulas; reference tables are for structure, scale, auditability, and long-term binding.
...
- dropdown Smart Cell binding to a reference table/range option column
