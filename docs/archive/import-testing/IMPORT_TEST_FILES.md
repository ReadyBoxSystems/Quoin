...
- The **Sheets** strip appears below the formula bar and above the grid.
...
- Imported supported Excel dropdown cells show dropdown options in the grid and inspector.
- Review items are visible when expected.
- Basic cross-Sheet formulas calculate across imported Sheets.
- Runner Preview shows surfaced Smart Cells, including supported imported dropdown inputs.
...
- Cross-sheet formulas on `Calculator` remain visible and calculate across preserved Sheets.
...
- `Calculator` formulas with `Inputs!...` references stay visible and calculate across preserved Sheets.
...
- Cross-sheet formula on `Summary` should remain visible and calculate across preserved Sheets.
...
- Quoted cross-sheet formulas remain visible and calculate across preserved Sheets.
...
- Cross-Sheet formulas on `Runner Calculator` calculate from `Runner Inputs`.
- No cross-Sheet review warnings are expected.
- After manually surfacing the promoted Smart Cells, Runner Preview shows cells from both Sheets grouped by Sheet.
...
- No cross-Sheet range review warnings are expected.
- After manually surfacing the promoted Smart Cells, Runner Preview shows cells from both Sheets grouped by Sheet.
...
- The supported cross-Sheet formula calculates.
- Cross-Sheet references are not review items by themselves.
- Review list still flags:
  - structured table reference
  - external workbook reference
  - semicolon argument separator
- After manually surfacing the promoted Smart Cells, Runner Preview shows cells from both Sheets grouped by Sheet.

### 16. Dropdown Import Fixture

File:

```text
quoin-dropdown-import-fixture.xlsx
```

Expected:

- Three Sheets appear:
  - `Dropdown Import Test`
  - `Lists`
  - `Reference Data`
- `B2:B6` on `Dropdown Import Test` become surfaced input Smart Cells with dropdown options.
- Dropdown sources covered:
  - literal list
  - same-Sheet range
  - cross-Sheet range
  - quoted Sheet range
  - workbook-defined named range
- Blank source values are skipped.
- Duplicate source values are deduplicated in first-seen order.
- `B7` remains a normal imported cell and produces a review item because `=Table1[Status]` is unsupported.
- Runner Preview shows the imported dropdown inputs as dropdown controls.
